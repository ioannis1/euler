...
{
    Complex    *euler = (Complex *) PG_GETARG_POINTER(0);
    StringInfoData buf;
    pq_begintypsend(&buf);
    pq_sendfloat4(&buf, euler->x);
    pq_sendfloat4(&buf, euler->y);
...
Datum
euler_polar(PG_FUNCTION_ARGS)
{
     // Convert euler from cartesian to polar coordinates
...
Datum
euler_angle_add(PG_FUNCTION_ARGS)
{
     Complex    *a = (Complex *) PG_GETARG_POINTER(0);
...
static int
euler_cmp_internal( Complex *a, Complex *b ) 
{
...
Datum
euler_new_polar(PG_FUNCTION_ARGS)
{
...
static int
euler_int2c_cmp_internal( int32 a, Complex *b ) 
{
